...
package ktu.display.align {
	 ...
	 * 		import com.crp.display.utils.AlignParam;																											<br/>
	 * 		var param:AlignParam = new AlignParam( VizAlign.LEFT, VizAlign.TO_TARGETS );
	 ...
	 * 		import com.crp.display.utils.VizAlign;
	 * 		import com.crp.display.utils.AlignParam;																											<br/>
	 * 		var param:AlignParam = new AlignParam( VizAlign.RIGHT, stage );
	 * 		VizAlign.align( [mc1, mc2], [ param ], true, stage );																								</listing><br/>
	 *																																							<br/>
	 *																																							<br/>
	 ...
	 * 		import com.crp.display.utils.VizAlign;
	 * 		import com.crp.display.utils.AlignParam;																											<br/>
	 * 		var param1:AlignParam = new AlignParam( VizAlign.SPACE_HORIZONTALLY, stage);
	 * 		var param1:AlignParam = new AlignParam( VizAlign.BOTTOM, VizAlign.TO_TARGETS);
	 ...
	 * 		import com.crp.display.utils.VizAlign;
	 * 		import com.crp.display.utils.AlignParam;																											<br/>
	 * 		var param1:AlignParam = new AlignParam (VizAlign.LEFT, VizAlign.TO_TARGETS);
	 * 		var param2:AlignParam = new AlignParam (VizAlign.TOP, VizAlign.TO_TARGETS);
	 ...
	 * paramater. Return array for this example would be: results[0] is an Array of AlignObjects, with
	...
	*/
	public class VizAlign extends AlignMethods {
		 ...
		 * 		import com.crp.utils.align.AlignParam;																											<br/>
		 * 		var param:AlignParam = new AlignParam ( VizAlign.LEFT, VizAlign.TO_TARGETS );
		...
		* 			applyResults() - Apply the resulting values on an Array of AlignObjects
		* 		   applyOriginal() - Apply the original values on an Array of AlignObjects
		...
		*/
		 ...
		 *
		 * @param	targets				Array of DisplayObject/AlignObject/Array	list of targets you wish to align
		 ...
		 * @return						Array of AlignObject	containing targets with end result of alignment math
		 *
		 *
		...
		public static function align (targets:Array, alignmentObjects:Array, applyResults:Boolean = false, stageRef:Stage = null):Array {
			...
			targetArray.every (convertToAlignObjects);					// convert targets to AlignObjects
			...
			for (var i:int = 0; i < length; ++i) {						// Loop through all of the alingments to be done on targets
				var p:AlignParam = alignmentObjects[i];					// save current align param
		...
		/**
		 *
		 * Apply End properties of AlignObjects to target
		 *
		 * @param		targets		Array of AlignObject		comma seperrated list of targets you wish to apply their end values
		 *
		 */
		...
		/**
		 *
		 * Apply Orig properties of AlignObjects to target
		 *
		 * @param		targets		Array of AlignObject		comma seperrated list of targets you wish to apply their original values
		 *
		 */
		...
		public static function cloneObject (target:DisplayObject):Sprite {
			return cloneTarget ( new AlignObject(target) );
		}
		/*
		**************************************************************************************************
		*
		...
		*/
		...
		private static function adjustGroups (targets:Array, groupResults:Array, newTargets:Array):Array {
			...
			for (var i:int = 0; i < length; ++i) {										// loop through each group we have
				var t:* = targets[i];
				if (t is AlignObject) {													// 		if target is AlignObject
		...
		private static function applyGroupMath (target:AlignObject, group:AlignObject, index:int):void {
		 ...
		 * @param	targets		all targets as AlignObject being used in this align() call.
		 */
		private static function moveAllTargets (targets:Array):void {
			...
			for ( var i:int = 0; i < length; ++i ) {
				if ( targets [ i ] is Array ) {					//if the element in the targets is an array (aka group)
					...
					AlignObject ( targets [ i ] ) .applyOrig ( ) ;	//apply the original results
				}
			}
		};
		/*		move to e
		...
		private static function moveAllToEnd (targets:Array):void {
			...
			for ( var i:int = 0; i < length; ++i ) {
				if ( targets [ i ] is Array ) {					//if the element in the targets is an array (aka group)
					...
					AlignObject ( targets [ i ] ) .applyEnd ( ) ;	//apply the end results
				}
			}
		};
		/*		round
		 ...
		 */
		private static function roundPixel ( targ:AlignObject ) :void {
		...
		*/
		...
		private static function validateCoordinateSpace ( targetCoordinateSpace:DisplayObjectContainer, targets:Array ) :DisplayObjectContainer {
			switch ( targetCoordinateSpace ) {
				case TO_TARGETS:
					...
					break;
				case _stageRef:
					targetCoordinateSpace = createStageSpace(targets);			//create a coordinate space for the stage
					createdSpace = true;
					break;
				default:
		...
		private static function createCoordinateSpace(targets:*):DisplayObjectContainer {
			...
			} else if (targets is AlignObject) {
		 ...
		 * @return
		 */
		private static function createStageSpace (targets:Array):DisplayObjectContainer {
			var obj:Sprite = new Sprite ();										// create container sprite
			obj.name = tempName;												// use the tempName for easy identification
		...
		*/
		/** @private
		 *
		 * 		cloneTarget will take an AlignObject and duplicate the visual bounding box in the target property.
		 * @param	targ
		 * @return
		 */
		private static function cloneTarget(targ:AlignObject):Sprite {
		...
		/** @private
		 *
		 * 		convertToAlignObjects will take all targets from align() call and convert them to AlignObjects if neccessary.
		 * If a target is already an AlignObject, then the target of that AlignObject is placed at the end values from previous
		 ...
		 * @return
		 */
		private static function convertToAlignObjects (element:*, index:int, arr:Array):* {
			switch (true) {
				case element is Array:
					element.every(convertToAlignObjects);	//if element is array, call this method again on that array
					haveGroups = true;						//set to true because an array was passed as a target, implying groups
					break;
				case element is DisplayObject:
					arr[index] = new AlignObject(element, index); //create align object with that element
					break;
				case element is AlignObject:
					AlignObject ( element ) .applyEnd ( ) ;		//if already AlignObject, place target at end values
					AlignObject ( element ) .uid = index;		//reset the uid to fit this align call
					break;
		 ...
		 * @param	passedRef
		 */
		private static function checkStageRef (targets:Array, passedRef:*):void {
		...
		*/
		...
		private static function checkDuplicateTargets (ar:Array):String {
			...
			for (var i:int = 0; i < length; ++i) {												// loop through all targets
				var outerI:DisplayObject = (ar[i] is AlignObject) ? ar[i].target : ar[i];		//		save ref to DisplayObject of target
				for (var j:int = 0; j < length; ++j) {											//		loop through all targets - check saved ref to all other targets
					...
					var innerJ:DisplayObject = (ar[j] is AlignObject) ? ar[j].target : ar[j];	// 			save ref to inner loop target
		...
		private static function checkTCSOverlap (targets:Array, alignmentObjects:Array):String {
			...
			for (var j:int = 0; j < length; ++j) {																// loop through targets passed in
				var t:DisplayObject = (targets[j] is AlignObject) ? targets[j].target : targets[j];				//		save the DisplayObject target
				for (var i:int = 0; i < alignlength; i++) {														// 		loop through every alignmentObject passed in
					if (AlignParam(alignmentObjects[i]).tcs == t){												//			if the tcs and target match we have a problem
...
class VizAlignError {
	...
	public static function e (type:String, extraInfo:String = "") :void {
		...
		switch (type) {
			...
			break;
			case BAD_TARGET:
				str = "VizAlign : Target must be a DisplayObject/AlignObject - " + extraInfo;
			break;
		}
