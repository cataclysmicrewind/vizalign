...
h2. powerful api:
	...
	the first thing to learn is that each alignment will need the objects you want to move and a stationary object to move the targets in relation to. then, understand the four primary objects in VizAlign and alignment will be easy.
	...
	therefore alignments can: alignment.align(targetRectangles:Array);
	...
	VizAlignTarget hold the target, its orignal dimensions, calculated dimensions and other settings
...
* *natural api*: i want to align these [_targets_] to the [_left of the stage_].
...
try reading the one liner above outloud like a sentence. if you can learn to think of your alignments like that VizAlign will be a breeze, and you'll want to raise your arms and just expel the stress into the wind. ahhh.

<br>

h2. origins:

in as3 (i hope) you know that each DisplayObject has its own coordinate plane. think about a DisplayObject like it is a piece of translucent grid paper with a coordinate plane drawn on it. you can draw your graphics anywhere you want on that paper. but no matter where you draw the graphics. flash will always think the .x and .y properties are at 0,0 of that coordinate plane! so if on your paper you draw a 4x4 box at position 5x5, and you place the paper at 0,0 on the stage (or parent), the 2x2 box will look 5 to the right and 5 down from 0,0 (flash's y coordinate is flipped from maths).
	
!=http://cataclysmicrewind.com/vizalign/examples/coordinatePlane.png!

this, is the reason i made VizAlign. if you have never encountered issues when trying to position graphics because of the origin point you are a lucky human being.
ignoring the origins of a target is implemented in the VizAlignTarget class, and can be turned on for every target through the third parameter of VizAlign.align().
