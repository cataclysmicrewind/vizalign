...
h2. powerful api:
	...
	the first thing to learn is that each alignment will need the objects you want to move (targets) and a stationary object (targetCoordinateSpace). alignments occur by placing the targets in relation to the targetCoordinateSpace. once you understand the four primary objects in VizAlign and alignment will be easy.
	
	each code statement below builds upon the previous, so if you are not sure what the variable referenced is, look up.

	IRectangleAligners align Rectangle. the only method align() needs a targetCoordinateSpace and targets.
	they need a targetCoordinateSpace (thing that doesn't move) and targets (things that do move).
