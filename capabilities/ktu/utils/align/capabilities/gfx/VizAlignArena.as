package ktu.utils.align.capabilities.gfx {
	...
	public class VizAlignArena extends Sprite {
		...
		private var _highlightTCS:Boolean = false;
		...
		public function get highlightTCS():Boolean {
			return _highlightTCS;
		}
		
		public function set highlightTCS(value:Boolean):void {
			_highlightTCS = value;
			var tcs:InteractiveObject;
			if (_highlightTCS) 
				for each (tcs in targetCoordinateSpaces)
					tcs.addEventListener(MouseEvent.ROLL_OVER, onTCSMouseOver);
			else
				for each (tcs in targetCoordinateSpaces)
					tcs.removeEventListener(MouseEvent.ROLL_OVER, onTCSMouseOver);
		}
	}
}